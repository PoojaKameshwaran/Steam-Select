...
clean_bundle_task = BashOperator(
    ...
    bash_command="python /opt/airflow/dags/data_preprocessing/clean_bundle.py",
    dag = dag
)

eda_bundle_task = BashOperator(
    task_id='eda_bundle_task',
    bash_command="python /opt/airflow/dags/data_preprocessing/EDA_bundle.py"
)

clean_items_task = BashOperator(
    task_id='clean_items_task',
    bash_command="python /opt/airflow/dags/data_preprocessing/clean_item_data.py"
)

eda_items_task = BashOperator(
    task_id='eda_items_task',
    bash_command="python /opt/airflow/dags/data_preprocessing/EDA_item_data.py"
)
